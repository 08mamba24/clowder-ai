...
`~` 为用户 HOME 占位。所有采集命令均含 `--config-dir <用户HOME>/.qoder-cn`（登录态）或显式空目录（auth-error）。
...
| permission-denial.jsonl | 权限拒绝 | `--tools "Write"`，提示写 `/tmp/pwned.txt`（未落盘） | 0 | 负向信号在 `user.tool_result{is_error:true,"Error: Allow ..."}`；**终态 `result.subtype:"success", is_error:false, permission_denials:[]` —— 拒绝在终态完全不可见** |
| auth-error.jsonl | 错误（未登录） | `--config-dir` 指向空目录 | 1 | `result.is_error:true` 但 **`subtype` 仍为 `"success"`** —— 判错必须以 `is_error` 为准 |
| silent-model-fallback.jsonl | 负向（静默回落） | 点点点采，`-m definitely-not-a-model-xyz` | 0 | stderr 警告 → `init.model=Auto` → `result.is_error:false` 成功返回 |
