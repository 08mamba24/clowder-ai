...
`~` 为用户 HOME 占位。**完整可复现命令、fixture setup、副作用断言与结构化 receipt 见 `collect.sh`（输出 `receipts.json`）；首采会话的 provenance 台账见 `PROVENANCE.md`（非原始 receipt，unknown 字段已标注）。**
...
| permission-denial.jsonl | 权限拒绝 | `--tools "Write"`，提示写 `/tmp/pwned.txt`（未落盘，见 PROVENANCE） | 0 | 负向信号在 `user.tool_result{is_error:true,"Error: Allow ..."}`；**终态结构化字段（`is_error:false, permission_denials:[]`）不可作为判拒依据；`result.result` 自由文本含 "was denied" 字样但不得作分类依据** |
| auth-error.jsonl | 错误（未登录） | `--config-dir` 指向空目录 | 1 | `result.is_error:true` 但 **`subtype` 仍为 `"success"`** —— 判错必须以 `is_error` 为准 |
| silent-model-fallback.jsonl | 负向（静默回落） | 点点点采，`-m definitely-not-a-model-xyz` | 0 | `init.model=Auto` → `result.is_error:false` 成功返回；stderr 回落警告属点点采集会话，仓内无该 stderr 工件（重跑 collect.sh 可补全，见 PROVENANCE） |
