...
export type ZcodeSpawnReadyResult = { ok: true } | { ok: false; error: Error };

/**
 * Hub spawn readiness for ZCode 0.16.3 app-server.
 * `ZCODE_MODEL` selects the model (session/new fails without it).
 * Provider calls still need ANTHROPIC_API_KEY or ZCODE_API_KEY; Desktop ~/.zcode/v2
 * and `zcode login` are not read by this Hub path.
 */
export function diagnoseZcodeSpawnReady(env: NodeJS.ProcessEnv): ZcodeSpawnReadyResult {
  if (!env.ZCODE_MODEL?.trim()) {
    return {
      ok: false,
      error: new Error(
        'ZCode ACP skipped: ZCODE_MODEL is required (catalog defaultModel is injected). session/new fails with model_config_missing without it.',
      ),
    };
  }
  if (!hasZcodeProviderCredential(env)) {
    return {
      ok: false,
      error: new Error(
        'ZCode ACP skipped: no provider credential. Bind a Hub API-key account to @zcode, or export ANTHROPIC_API_KEY before starting Hub. Desktop ~/.zcode/v2 and zcode login are not used; do not write ~/.zcode/cli/config.json.',
      ),
    };
  }
  return { ok: true };
