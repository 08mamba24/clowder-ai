...
- F167 留痕：谱谱三轮 invocation 的 callback 工具面均不可用（`$CAT_CAFE_API_URL` / `INVOCATION_ID` / `CALLBACK_TOKEN` 未注入），`complete_a2a_dispatch` 无法调用，处置走行首 @ 通道并在此留痕；推送依赖持 gh 凭证的猫（本轮=砚砚6）。
