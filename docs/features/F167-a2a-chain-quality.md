...
### Case E22: 把修复推进截断为诊断交付与重复审批（2026-09-21，astra）

| 维度 | 内容 |
|------|------|
| 我以为 | 两只隔离载体的 GitHub 查询根因解释完即可收线；被要求制定修改计划后，又把既定两仓只读范围笼统挂为待 operator 批准。 |
| 实际要求 | 在连续修复意图内主动制定并推进计划，技术选型、测试和 review 由猫猫完成；只有实际需要扩大范围、增加写权限或放松隔离时才提交新决策。runtime 配置/重启若必须由人操作，应在可执行版本与验收就绪后交精确操作单。 |
| 偏差根因 | **阶段产物替换终态 + 授权、工程验证、部署操作混同**：先把“根因回答完成”当成“修复意图已处理”，再把尚未实现/未验收误写为授权不足。与 Case E3 的“完成一段工作即停在汇报”同型。分界判据：先检查当前会话已授权的任务范围，再分别判断技术未知、代码门禁和人类专属操作；不能用最后一步可能需要人操作阻塞当前可执行工作，也不能把质疑审批的话伪造为新批准事件。 |
| 纠正轮次 | 同一修复任务 2 次：thread `thread_msqw8n1bqpvmob6f` 的 `0001789954443843-000354-15718ecc`「不制定计划修改吗」；`0001789955401202-000363-924099ab`「需要我决策什么？真的需要我决策吗？」。首次后计划以 `e24c334ed` 落盘；第二次后修正该计划的状态、范围、Task 5 和交接，并开始记录 ZCode 原生接缝调查；未声称代码修复或 live 验收已完成。 |
| 元心智哪条没执行 | Q1 当前在做什么：没有保持修复意图跨阶段连续；Q3 方案是否笨重：把工作内的工程责任反复转成 operator 选择题。 |
