...
### LL-104: 思考过程吐进正文——推理只进思考通道，正文只放结论、证据与状态

- 状态：draft
- 更新时间：2026-09-16

- 坑：thread_msqw8n1bqpvmob6f（CodeBuddy 账单 spike，同日已产 LL-103）里，zcode（glm-5.3）把内部思考过程整段吐进可见消息正文，operator 纠正原话"你又把思考过程吐出来了"——"又"表明至少第二次同类纠正。泄漏原文本随 session 封存未保留，本条以纠正原话为锚；防护按"正文推理独白"形态设计，若下次出现的是字面 think-tag 泄漏则属序列化层问题，另行定位。
- 根因：三层失守。①把"展示推理过程"当成严谨透明——实际读者要结论先行，透明应由证据锚点表达，不由推理独白表达；②思考通道/正文边界失守——探索性推理（"让我看看…可能是…那再试试…"）只该存在于思考通道，生成正文时被复述出来；③工具调用之间的"简短状态"膨胀成完整推理段——规范允许的是一句"接下来做什么"，实际写出的是"为什么 + 还有什么可能"。
- 触发条件：多工具长任务的进度播报；不确定/冷启动时用自言自语式探索填补正文；试图向 operator 证明"我在认真查"。
- 修复：operator 纠正当即认账。行为约束即刻生效：正文只放四类内容——结论、证据锚点、下一步动作、一句以内的状态句；其余全部进思考通道。
- 防护：
  1. **独白词自检**：正文出现"让我 / 嗯 / 可能是 / 那试试 / 首先…然后…"式独白词即删——那是思考通道内容漏进正文。
  2. **状态句只写"做什么"**：不写"为什么 / 还可能"——why 属于思考通道或最终总结。
  3. **删除测试**：一段正文删掉后读者决策不变 → 它不该在正文里。
  4. **纠正即锚**：冷启动被纠正时，纠正原话本身就是归档锚点，不因"找不回原文"推迟归档。
- 来源锚点：thread_msqw8n1bqpvmob6f operator 纠正原话"你又把思考过程吐出来了"（2026-09-16，本轮）| LL-103（同 thread 同日姊妹教训）
- 原理：**正文是给读者的交付物，思考通道是自己的工作台。** 把工作台搬进交付物不是透明——透明靠证据锚点，不靠旁观推理；推理独白对读者是纯噪音，还占 operator 的阅读时间。

- 关联：LL-103（同日同 thread）| glm-5.3 binding（binding 更换需复验本条倾向）| 家规"实事求是"的证据锚点纪律
