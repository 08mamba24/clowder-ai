...
- **open（已部分闭合，2026-09-16 15:1x CST 判别实验）**：operator 指路"用不免费的模型对照"后，同口径最小调用（"Reply with exactly: ok"，ACP 通道，HOME 注入）三组实测——**hy4-preview-f：`usage.credit = 0`（免费期实锤，server 自报）**；**deepseek-v4.1-flash：`credit = 0.67`**（23,465 tokens）；**glm-5.3：`credit = 2.48`**（22,684 tokens，≈ ds41 的 3.7 倍，"贵得更明显"如 operator 所料）。`usage_update.cost.amount` 与 `usage.credit` 同值，**计价单位即积分**（与 WorkBuddy 积分面板同币种）。粗略费率（单样本、completion 极小、cached_tokens=0）：ds41 ≈ 0.029 积分/1k tokens，glm-5.3 ≈ 0.109 积分/1k tokens；hy4-preview-f = 0。仍 open：长对话/大 completion 档位未采样；面板侧余额对账（本轮两次计费调用应消耗 0.67+2.48=3.15 积分，operator 可在"套餐与积分"页复核，兼作面板↔协议对账）。
